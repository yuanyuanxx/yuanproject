...
def create_Config():
    ...
    config.add_section("hostsuffix2")

    # 在新增的section下加key-value键值对
    config.set("qa-test", "user", "zhouyuan")
    config.set("qa-test", "host", "api.douban.com")
    config.set("hostsuffix", "doubanhost", "/v2/book/search?")
    config.set("hostsuffix2", "doubanhost2", "/v2/book/search?")
...
'''
@pytest.mark.usefixtures("create_Config")
def test_get_doubanhost():
    config.read("config.ini")
    doubanhost = config.get('hostsuffix', 'doubanhost')
    return doubanhost
